...
module reorder_buffer(
    ...
    // =========================================================
    function logic [PTR_W-1:0] next_ptr(input logic [PTR_W-1:0] p);
        logic [PTR_W-1:0] n;
        n = p + PTR_W'(1);
        return (n == '0) ? PTR_W'(1) : n;
    endfunction
    ...
    always_ff @(posedge clk) begin
        ...
        for (int i = 0; i < `ROB_DEPTH; i++) begin
            if (rob[i].valid && 
                (PTR_W'(i) != head) &&
                (PTR_W'(i) == flush_seq_i[PTR_W-1:0])) begin
                rob[i].valid  <= 1'b0;
                rob[i].filled <= 1'b0;
                rob[i].done   <= 1'b0;
            end
        ...
        count <= (PTR_W+1)'(flush_seq_i[PTR_W-1:0] - head);
            ...
            unique case ({do_commit, (id2rob_valid_i && (id2rob_seq_num != '0))})
