...
module vector_execution_unit(
    ...
    input   logic                               Ctrl_add, mult_start,
    input   logic [6:0]                         sew_eew_mux_out,
    input   logic [2:0]                         execution_op,
    input   logic                               signed_mode_mult, 
    input   logic                               mul_low,
...
logic [`Tag_Width-1:0]                          seq_num_held;
    ...
    logic                               sew_16_32;
    logic                               sew_32;
    logic                               add_en,     shift_en,   mult_en,    compare_en,     bitwise_en,     reverse_sub_en, 
                                        move_en,    mask_add_en,    mult_add_en;
    logic                               sum_done,   shift_done,     compare_done,   bitwise_done,   product_sum_done,   move_done, 
                                        sum_mask_done;
    logic                               sum_done_internal,  shift_done_internal,    mult_done_internal,     compare_done_internal,  
                                        sum_mask_done_internal, bitwise_done_internal, product_sum_done_internal;
    ...
    logic [`VLEN-1:0]                   sum_result,     compare_result,     bitwise_result,     shift_result,   move_result, 
                                        sum_mask_result,    sum_product_result; 
    ...
    always_ff @(posedge clk) begin
        if (!reset) begin
            ...
            accum_inst_latch    <=1'b0;
                ...
                sew_latch           <= sew;
                Ctrl_latch          <= Ctrl_add;
                signed_sum_mode_reg <= signed_mode_mult;
    ...
    logic signed_mult_sum, signed_mult;
    ...
    assign signed_mult_sum = (signed_mode_mult) ? 1'b1 : signed_sum_mode_reg;
    ...
    always_ff @(posedge clk) begin
        if (!reset) begin
            ...
            signed_mode_reg <= 1'b0;
                ...
                signed_mode_reg <= signed_mode_mult;
        end else if (mult_done_internal) begin
                mult_data_1_reg <= '0;
                mult_data_2_reg <= '0;
                mult_active     <= 1'b0; 
    ...
    assign signed_mult          = (signed_mode_mult) ? 1'b1 : signed_mode_reg; 
        ...
        sum_mask_done = 1'b0;
            if (reset) begin  
                ...
                if (add_en | reverse_sub_en) begin
                    ...
                    execution_result = {{(`MAX_VLEN-`VLEN){1'b0}}, sum_result};
                ...
                if (shift_en) begin
                    execution_result = {{(`MAX_VLEN-`VLEN){1'b0}}, shift_result};
                ...
                if (compare_en) begin
                    execution_result = {{(`MAX_VLEN-`VLEN){1'b0}}, compare_result};
                ...
                if (bitwise_en) begin
                    execution_result = {{(`MAX_VLEN-`VLEN){1'b0}}, bitwise_result};
                ...
                if (move_en) begin
                    execution_result = {{(`MAX_VLEN-`VLEN){1'b0}}, move_result};
                    move_done = 1'b1;  
                    sum_done = 1'b0;
                    shift_done = 1'b0;
                    mult_done = 1'b0;
                ...
                if (mask_add_en | mult_sum_active) begin
                    ...
                    execution_result = {{(`MAX_VLEN-`VLEN){1'b0}}, sum_mask_result};
                ...
                if (mult_done_internal && product_sum_done_internal && sum_done_internal && accum_inst_latch) begin
                    ...
                    bitwise_done = 1'b0;
                    sum_mask_done = 1'b0;
                    execution_result = {{(`MAX_VLEN-`VLEN){1'b0}}, sum_product_result};
                end
            end
            else begin 
                execution_result = '0;
...
(* keep_hierarchy = "yes" *)    vector_adder_subtractor adder_inst (
        .A              (adder_data_1),
        .B              (adder_data_2),
        .Ctrl           (Ctrl_add),         
        .sew_16_32      (sew_16_32),     
...
(* keep_hierarchy = "yes" *)     vector_multiplier vect_mult(
        .clk            (clk),
        .reset          (reset),
...
(* keep_hierarchy = "yes" *)     vector_compare_unit vect_comp (
...
(* keep_hierarchy = "yes" *)     vector_bitwise_unit vect_bitwise (
...
(* keep_hierarchy = "yes" *)     vector_shift_unit vector_shift(
        .dataA          (shift_data_1),         
        .dataB          (shift_data_2),             
        .shift_op       (shift_op),      
        .sew            (sew),           
...
(* keep_hierarchy = "yes" *)     vector_multiply_add_unit mult_add(
        .clk                (clk),
        .reset              (reset),
...
(* keep_hierarchy = "yes" *)     vector_mask_add_sub mask_add_sub (
