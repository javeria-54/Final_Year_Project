...
module lsu (
    ...
    // EXE <---> LSU interface
    input   var  type_exe2lsu_data_s               exe2lsu_data_i,
    input   var  type_exe2lsu_ctrl_s               exe2lsu_ctrl_i,            // Structure for control signals from execute to memory 

    // LSU <---> CSR interface
    output  var type_lsu2csr_data_s               lsu2csr_data_o,
    output  var type_lsu2csr_ctrl_s               lsu2csr_ctrl_o,

    // AMO <---> LSU interface
    input   var  type_amo2lsu_data_s               amo2lsu_data_i, 
    input   var type_amo2lsu_ctrl_s               amo2lsu_ctrl_i,              
    output  var type_lsu2amo_data_s               lsu2amo_data_o,
    output  var type_lsu2amo_ctrl_s               lsu2amo_ctrl_o,

    // LSU <---> WRB interface
    output  var type_lsu2wrb_data_s               lsu2wrb_data_o,
    output  var type_lsu2wrb_ctrl_s               lsu2wrb_ctrl_o,
    ...
    output  var type_lsu2fwd_s                    lsu2fwd_o,
    input   var type_fwd2lsu_s                    fwd2lsu_i,

    output logic                             lsu_done_o, 
    input logic [`XLEN-1:0]                  rob_commit_scalar_mem_data,
    input logic                              rob_commit_scalar_rd_wr_req,
    input  var  type_st_ops_e                      rob_commit_scalar_store_op,
    input [`XLEN-1:0]                        rob_commit_scalar_mem_addr,
    // LSU <---> Data Bus (dbus) interface
    input   var type_dbus2lsu_s                   dbus2lsu_i,
    output  var type_lsu2dbus_s                   lsu2dbus_o,                // Signal to data bus 
